...
export const CROSS_SYSTEM_SEMANTIC_STREAM_CONTRACTS: CrossSystemSemanticStreamContract[] = [
  ...
  },
  {
    ...
    grain: "统计日 × 店铺 × 天猫平台 SKU",
    controls: {
      grain: implemented("业务键固定为 statisticalDate + shopName + platformSkuId"),
      business_time: implemented("statisticalDate 是平台统计业务日，与同步时间分开"),
      identifier_namespace: reviewPending("平台 SKU、子货品编码和条码已分列保留；子货品编码目前只作认领候选", "由商品团队确认子货品编码稳定性并完成冲突 UAT"),
      quantity_unit: implemented("paidNumber、successRefundSuborderNumber 与 unitQty 保留为源系统件数"),
      currency: implemented("支付与退款金额仅按来源原币观察，不跨币种混加"),
      amount_scale: implemented("支付与退款金额按两位小数验证并保留十进制语义"),
      correction_semantics: reviewPending("源表已分列销售与成功退款，但历史表已停更", "仅作身份和历史旁证，不用它替代当前销售事实"),
    },
  },
  {
    source: "JIANDAOYUN",
    ...
    grain: "拼多多订单 × 商品规格",
    controls: {
      grain: implemented("业务键固定为 orderNumber + productId + merchantSkuCode"),
      business_time: implemented("统计日、付款时间和发货时间分列，需求窗口使用统计日"),
      identifier_namespace: reviewPending("平台商品与商家规格编码分列保留，尚未证实与 SCM 编码同域", "只允许通过拼多多对照表中的唯一映射进入 SKU 聚合"),
      quantity_unit: reviewPending("productQuantity 是订单规格件数，尚未证明与 SCM 基础单位一致", "完成与商品对照表的单位换算 UAT 后再形成正式净需求"),
      status_semantics: reviewPending("取消/退款成功已从观察需求排除，其余源状态仍待业务冻结", "由电商团队确认支付、取消、部分退款和补发状态集"),
      correction_semantics: reviewPending("观察行不可变保留，但订单改单/售后版本关系尚未冻结", "冻结源端更新与售后关联后完成净需求对账"),
    },
  },
  {
    source: "JIANDAOYUN",
    ...
    grain: "统计日 × 唯品会店铺 × 品牌",
    controls: {
      grain: implemented("业务键固定为 statisticalDate + shopName + brandName"),
      business_time: implemented("statisticalDate 是平台统计业务日"),
      quantity_unit: implemented("salesQuantity、customerNumber 和 subOrderNumber 分列保留，不互换"),
      currency: reviewPending("销售额和优惠券金额已保留，但表单没有显式币种字段", "确认店铺结算币种；确认前不与其他币种汇总"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      status_semantics: reviewPending("当前是店铺交易汇总，没有单独订单状态", "由电商团队确认销售量/销售额是否已净退货"),
    },
  },
  {
    source: "JIANDAOYUN",
    ...
    grain: "统计日 × 店铺 × 天猫平台商品",
    controls: {
      grain: implemented("业务键固定为 statisticalDate + shopName + platformProductId"),
      business_time: implemented("statisticalDate 是平台统计业务日"),
      identifier_namespace: reviewPending("平台商品 ID 已保留，当前不能唯一下沉到 SCM SKU", "建立商品→SKU 受控桥后再提供 SKU 级损益"),
      quantity_unit: implemented("paidNumber 与 successRefundOrderCount 分列保留，不将退款订单数冒充退货件数"),
      currency: reviewPending("成交额、销售费用与预估利润已保留，表单没有显式币种字段", "确认店铺结算币种后再跨店聚合"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      sign_convention: reviewPending("成交、退款、费用与利润分列保留，负值意义尚未业务验收", "核对冲销、退回与负毛利样本，不取绝对值"),
      correction_semantics: reviewPending("原行按源记录不可变保留，跨日回补/调整语义尚未冻结", "用完整月控制总量核对后再用于正式毛利决策"),
    },
  },
  {
    source: "JIANDAOYUN",
  ...
  // 2026-09-03 第四阶段：聚水潭镜像 / 财务成本 / 平台组合对照 / 流量先行指标
  {
    ...
    grain: "聚水潭商品编码（单品）",
    controls: {
      grain: implemented("业务键为聚水潭商品编码"),
      business_time: implemented("创建/修改时间为聚水潭主数据时间，不是同步时间"),
      identifier_namespace: implemented("聚水潭编码保留在 JST 命名空间，不与 SCM 系统编码混同"),
      quantity_unit: implemented("本流无金额字段"),
      currency: implemented("本流无金额字段"),
      amount_scale: implemented("本流无金额字段"),
  ...
  },
  {
    ...
    grain: "聚水潭组合商品编码 × 子商品编码",
    controls: {
      grain: implemented("业务键为组合商品编码 + 子商品编码"),
      business_time: implemented("创建/修改时间为聚水潭主数据时间"),
      identifier_namespace: implemented("聚水潭编码保留在 JST 命名空间，不与 SCM 系统编码混同"),
      quantity_unit: implemented("数量为组合装内子商品件数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: reviewPending("组合成本价/子商品成本价在源端为文本列", "确认是否为金额并固定精度后再做成本口径"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
    },
  },
  {
    ...
    grain: "店铺 × 天猫货品 ID × 子货品 ID",
    controls: {
      grain: implemented("业务键为 shopName + productId + subproductId"),
      business_time: implemented("主数据快照，无业务日"),
      identifier_namespace: reviewPending("系统编码列已分列保留；是否与 SCM skus.code 完全同义待商品团队确认", "按 exactHits 命中率与冲突样本做 UAT"),
      quantity_unit: implemented("数量为组合内子货品件数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
    },
  },
  {
    ...
    grain: "拼多多商家编码（规格维度）",
    controls: {
      grain: implemented("业务键为商家编码-规格维度"),
      business_time: implemented("成本标准快照，无业务日"),
      identifier_namespace: implemented("商家编码、匹配编码、聚水潭编码、国际条码四列分列保留，不混同"),
      quantity_unit: implemented("主商品件数为组合件数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
    },
  },
  {
    ...
    grain: "唯品会上架条码 × 对应系统编码",
    controls: {
      grain: implemented("业务键为 vipBarcode + systemSkuCode"),
      business_time: implemented("对照快照，无业务日"),
      identifier_namespace: reviewPending("系统编码列已分列保留；是否与 SCM skus.code 完全同义待商品团队确认", "按 exactHits 命中率与冲突样本做 UAT"),
      quantity_unit: implemented("关联单品数量为组合件数"),
      currency: implemented("本流无金额字段"),
      amount_scale: implemented("本流无金额字段"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
    },
  ...
  },
  {
    ...
    grain: "系统编码 × 适用月份 × 适用情况",
    controls: {
      grain: implemented("业务键为 productCode + useMonth + usage"),
      business_time: implemented("适用月份为成本生效月，不是录入时间"),
      identifier_namespace: reviewPending("系统编码列已分列保留；是否与 SCM skus.code 完全同义待商品团队确认", "按 exactHits 命中率与冲突样本做 UAT"),
      quantity_unit: implemented("本流无金额字段"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: reviewPending("同一编码多月份并存，取用规则待财务确认", "财务确认「最近适用月份优先」后再进入毛利口径"),
    },
  },
  {
    source: "JIANDAOYUN",
    stream: "vip-sku-cost-observation",
    grain: "唯品会商品 ID × 条码 × 系统编码 × 适用月份",
    controls: {
      grain: implemented("业务键为 productId + vipBarcode + systemSkuCode + useMonth"),
      business_time: implemented("适用月份为成本生效月"),
      identifier_namespace: reviewPending("系统编码列已分列保留；是否与 SCM skus.code 完全同义待商品团队确认", "按 exactHits 命中率与冲突样本做 UAT"),
      quantity_unit: implemented("关联单品数量为组合件数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
  ...
  },
  {
    ...
    grain: "统计日 × 店铺 × 天猫 SKU ID",
    controls: {
      grain: implemented("业务键为 statisticalDate + shopName + skuId"),
      business_time: implemented("statisticalDate 为平台统计业务日"),
      identifier_namespace: reviewPending("SKU ID 与关联货品分列；关联货品是否等于系统编码待确认", "以 exactHits 冲突样本做 UAT"),
      quantity_unit: implemented("支付件数/子订单数为源系统件数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("销售与成功退款分列，退款不冲减销售列"),
    },
  },
  {
    source: "JIANDAOYUN",
    stream: "tmall-product-traffic-observation",
    grain: "统计日 × 店铺 × 天猫商品 ID",
    controls: {
      grain: implemented("业务键为 statisticalDate + shopName + productId"),
      business_time: implemented("statisticalDate 为平台统计业务日；90 天服务端时间窗"),
      identifier_namespace: implemented("只有商品 ID，宝贝级"),
      quantity_unit: implemented("访客/浏览/件数均为源系统计数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("时间窗滚动快照；读模型跨批次按业务键去重，取最新批次的值"),
    },
  },
  {
    source: "JIANDAOYUN",
    stream: "pdd-product-daily-observation",
    grain: "统计日 × 店铺 × 拼多多商品 ID",
    controls: {
      grain: implemented("业务键为 statisticalDate + shopName + productId"),
      business_time: implemented("statisticalDate 为平台统计业务日；90 天服务端时间窗"),
      identifier_namespace: implemented("只有拼多多商品 ID，商品级"),
      quantity_unit: implemented("访客/浏览/成交件数均为源系统计数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("时间窗滚动快照；读模型跨批次按业务键去重，取最新批次的值"),
    },
  },
  {
    ...
    grain: "统计日 × 拼多多店铺",
    controls: {
      grain: implemented("业务键为 statisticalDate + shopName"),
      business_time: implemented("statisticalDate 为平台统计业务日"),
      identifier_namespace: implemented("店铺名称为来源作用域标识"),
      quantity_unit: implemented("订单数/买家数为源系统计数"),
      currency: implemented("金额仅按来源人民币观察，不跨币种混加"),
      amount_scale: implemented("金额按两位小数验证并保留十进制语义"),
      correction_semantics: implemented("成交与成功退款分列，退款不冲减成交列"),
    },
  },
  {
    source: "JIANDAOYUN",
    ...
    grain: "店铺名称",
    controls: {
      grain: implemented("业务键为店铺名称"),
      business_time: implemented("主数据快照，无业务日"),
      identifier_namespace: implemented("销售平台编码、品牌 ID、聚水潭店铺编号分列保留"),
      quantity_unit: implemented("本流无金额字段"),
      currency: implemented("本流无金额字段"),
      amount_scale: implemented("本流无金额字段"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
  ...
  },
  {
    ...
    grain: "品牌 ID",
    controls: {
      grain: implemented("业务键为品牌 ID"),
      business_time: implemented("主数据快照，无业务日"),
      identifier_namespace: implemented("品牌 ID 为数据中台命名空间"),
      quantity_unit: implemented("本流无金额字段"),
      currency: implemented("本流无金额字段"),
      amount_scale: implemented("本流无金额字段"),
      correction_semantics: implemented("每批是主数据全量快照，读模型只取最新批次；不做增量纠错"),
