/**
 * 上游删除墓碑：让「同步因为少了一条记录而永久停摆」变成一次可签字的决定。
 *
 * 事故实况（2026-09-04 → 09-05）：`jst-item-master-mirror-observation` 从 6448 掉到 6447，
 * 同步每轮拒绝替代批次 #524。拒绝是对的——没有墓碑就分不清「上游删了一条」与
 * 「权限/分页缩了，只看得到一部分」，后者当成前者接受，观察基线会被悄悄削掉一截。
 * 问题在于当时**没有任何让人确认的路径**：唯一在跑通的连接器就此停摆，只能改代码才能恢复。
 *
 * 本模块补的就是那条路径，且刻意保持窄：
 *  - 一次签字只放行**一条**具体记录，不存在「以后丢的都算数」；
 *  - 只能为**真的出现过**的记录签字（回查 staging_rows）——不能给系统没见过的 ID 预签；
 *  - 必填依据；仅管理员；同事务写审计；签错了能撤销，同样留痕。
 *  - **墓碑不改变截断判定**：缺失若呈「尾部整段消失」的形状，即使逐条签了字也照样拒绝
 *    （那不是删除，是截断）。这条在 jiandaoyun-sync 里执行，本模块只负责「谁签了什么」。
 */
