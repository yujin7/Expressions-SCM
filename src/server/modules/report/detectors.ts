...
const EMPTY_SUMMARY: DetectorSummary = { salesStop: 0, channelShift: 0, velocity: 0, scanned: 0 };
...
export async function getDetectorAlerts(
  ...
  const summary: DetectorSummary = { salesStop: 0, channelShift: 0, velocity: 0, scanned: skuRows.length };
    ...
    if (vel.changed) {
      ...
      all.push({
        ...
        detail:
          ...
          `偏离 ${vel.deviationPct}%（阈值 ±${DETECTOR_THRESHOLDS.velocityDeviation * 100}%）。两者除数不同（自然月 vs 91 天窗口），小幅偏离属口径噪音，请以趋势看待`,
      });
    }
  }

  /* ── 筛选/排序/分页（严重度优先，其次在库量——货多的先看） ── */
  let filtered = all;
  if (query.kind) filtered = filtered.filter((r) => r.kind === query.kind);
  ...
  const KIND_ORDER: Record<DetectorKind, number> = { sales_stop: 0, channel_shift: 1, velocity: 2 };
  filtered = [...filtered].sort(
    (a, b) => SEV_ORDER[a.severity] - SEV_ORDER[b.severity] || KIND_ORDER[a.kind] - KIND_ORDER[b.kind] || b.onHand - a.onHand,
  );
