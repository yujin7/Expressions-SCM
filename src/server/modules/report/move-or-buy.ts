...
/**
 * `none` = 既不用买也没货可挪（补货引擎给出 `suggestQty = "0"` 且无调拨建议）。
 * 此前这种行被判成 `transfer_only` 并计入「先挪即可（无需采购）」——一条**一件都挪不了**的行
 * 在汇总里冒充「已被调拨覆盖」，`coveredByTransfer` 因此虚高。
 */
export type MoveOrBuyAction = "transfer_only" | "transfer_then_buy" | "buy_only" | "none";
