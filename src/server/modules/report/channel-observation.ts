...
import { dAdd, dSub } from "@/server/core/decimal";
...
const money = (v: unknown): string => { const t = v == null ? "" : String(v).trim(); return /^-?\d+(\.\d+)?$/.test(t) ? Number(t).toFixed(2) : "0.00"; };
...
export async function computeChannelObservation(db: ReadDb): Promise<ChannelObservation> {
  ...
  let pdd = insufficient("拼多多", "订单 × 商品 × 商家编码（3 天滚动快照去重累加）", "拼多多订单流尚未同步。");
  {
    ...
    if (anchor) {
      ...
      let units = 0;
      const byShop: ChannelPlatformRow["byShop"] = [];
      for (const x of rows) {
        if (x.kind !== "shop") continue;
        const shop = String(x.shop ?? ""); const q = num(x.qty);
        units += q; byShop.push({ shop, units: q, amount: null });
        const b = brandOfShop(shop, brands); byBrand.set(b, (byBrand.get(b) ?? 0) + q);
      }
      pdd = {
        ...
        byShop: byShop.sort((a, b) => b.units - a.units),
        gate: "有效订单件数（剔除已取消/退款成功）；订单流无金额字段。窗口内批次不足 30 天时件数偏低。",
      };
    }
  }
  ...
  if (pnl) {
    ...
    const sorted = [...products].sort((a, b) => Number(b.estimatedNetProfit) - Number(a.estimatedNetProfit));
    productPnl = {
      state: anchor && products.length ? "ready" : "insufficient",
      sourceAsOf: pnl.sourceAsOf, anchorDate: anchor,
      totals,
      topNetProfit: sorted.slice(0, 10),
      bottomNetProfit: sorted.filter((p) => Number(p.estimatedNetProfit) < 0).slice(-10).reverse(),
