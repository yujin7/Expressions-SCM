...
import { ctCreateStorageKey, clearCtCreateRequest, loadCtCreateRequest, lookupCtCreateRequest,
  prepareCtCreateRequest, submitCtCreateRequest, withCtCreateLock,
...
export function useCtCreateRecovery(actorId: number | null, allowed: boolean, onConfirmed: () => void) {
  ...
  const acknowledge = () => run(async id => {
    ...
    if (active()) show(clearCtCreateRequest(localStorage, id, request.requestKey));
  });
  return { request, result, error, ready, busy, lookup, submit, acknowledge };
}

export default function CtCreateRecovery({ recovery, onEdit }: {
  recovery: ReturnType<typeof useCtCreateRecovery>; onEdit?: (request: CtCreateRequest) => void;
}) {
  ...
  return <Alert style={{ marginBottom: 12 }} type={error ? "error" : result?.document ? "success" : "warning"} showIcon
    message={result?.document ? "已找到原采购退货单，请核对当前状态" : "采购退货建单结果待核对"}
    description={<Space direction="vertical" size={8} style={{ width: "100%" }}>
      ...
      {request && <>
        ...
        <Space wrap size={8}>
          ...
          {!result?.document && !onEdit && <Link href="/matflow/ct">前往采购退货单据修正原请求</Link>}
