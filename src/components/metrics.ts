...
export const METRICS: Record<string, MetricDef> = {
  ...
  bondedOutboundDaily: {
    ...
    formula: "Σ保税订单.发货数量（全量归档观察；可用批次按源记录去重取最新状态；按发货日汇总，剔除状态含「取消」的订单）",
    unit: "qty",
    tier: "registry",
    caveat: "保税仓是快照仓，本序列来自订单流不是仓库流水：不过账、不进销速、不能与快照在库相减推算期初；7/30 天统计以最新有效发货日而非今天为锚点，须结合源数据日期判断新鲜度；订单状态语义待业务确认",
  },
  expiryByBrand: {
    id: "expiryByBrand",
    label: "效期分布（按品牌）",
...
Object.assign(METRICS, {
  ...
  /* ── BI wave 2 驾驶舱趋势块（C2/C3/C5/C6、B5/B8） ── */
  supplierSpendConcentration: {
    id: "supplierSpendConcentration",
    label: "供应商采购额集中度",
    short: "当年采购额最高的前 5 家供应商占全部供应商采购额的比例",
    formula: "Σ前 5 家当年采购额 ÷ Σ全部供应商当年采购额 × 100；采购额 = 该年已批 PO 行未税金额 + 该年生效 JS 结算金额",
    unit: "pct",
    tier: "derived",
    caveat: "占比全员可见、金额只对采购/PMC/财务/管理员可见；分母是采购与结算口径不是应付；集中度只描述结构不等于风险，要与 OTIF、账期和替代性一起看",
  },
  poMonthlyOtif: {
    id: "poMonthlyOtif",
    label: "逐月 OTIF",
    short: "按下单月归期的准时足量交付率，判定与年度累计完全同口径",
    formula: "该月已批 PO 中「全收完成日 ≤ 承诺日 + otif_window_days 且累计已收 ≥ 应收 ×(1 − 容差)」的张数 ÷ 该月可评张数",
    unit: "pct",
    tier: "derived",
    caveat: "近月的 PO 多半还没到承诺日，可评 n 结构性偏低——每个点必须带 n，可评为 0 的月留空、绝不写成 0%；缺承诺交期进「不可评」不进分母",
  },
  expirySlowMoverOnHand: {
    id: "expirySlowMoverOnHand",
    label: "呆滞在库（按品牌）",
    short: "被风险工作台判为「滞销关注」的 SKU 在各品牌的数量与在库合计",
    formula: "可销天数 ≥ slow_days_threshold（缺省 180 天）且在库 > 0 的 SKU，按品牌计数并合计在库",
    unit: "qty",
    tier: "derived",
    caveat: "可销天数按近 3 月月度销量折日均，无动销记为可销无限大；跨 SKU 数量直加只作规模参考；外部近 30 天仍在卖只是观察注记，不是「可以处置」或「不能处置」的结论",
  },
  tierMigration: {
    id: "tierMigration",
    label: "分层迁移",
    short: "相邻两个固化期之间 S/A/B/C 之间换档的 SKU 数",
    formula: "对两期 sku_planning_policy 取 coalesce(override_tier, tier) 做 5×5 交叉（S/A/B/C/未分层），换档数 = 起止分层不同的 SKU 数",
    unit: "count",
    tier: "derived",
    caveat: "只在某一期出现的 SKU 落在「未分层」轴，不假装它有分层；人工覆写计入生效分层，所以迁移里既有规则变化也有人工调整，两者在本表不区分",
  },
  pilotBlockerFunnel: {
    id: "pilotBlockerFunnel",
    label: "试点阻塞漏斗",
    short: "没进补货试点候选的 SKU 卡在哪一维——周期缺失、波动、异动还是分层",
    formula: "对 S/A/B 逐维计数：加工/在途周期未维护、XYZ 未分类（样本不足）、XYZ 非 X、异动命中；C 级单列",
    unit: "count",
    tier: "derived",
    caveat: "一个 SKU 可同时命中多维，各桶不可相加成总数；XYZ 未分类是「样本不足」不是「波动大」，必须单列不能并进「非 X」；周期缺失是主数据缺口，补录后候选立即生效",
  },
  dataFreshnessAgeDays: {
    id: "dataFreshnessAgeDays",
    label: "数据滞后天数（周趋势）",
    short: "每周各来源类最陈旧的一次入库，落后业务截止日多少天",
    formula: "按 ISO 周（Asia/Shanghai 周一起）取 max(import_jobs 收到日 − source_as_of)，按 D65 来源类分组，窗口 8 周",
    unit: "days",
    tier: "ledger",
    caveat: "没有业务截止日（source_as_of）的批次不参与，不按 0 处理；8 周里少于 3 周有入库或运行的来源类不出趋势；这是「入库时的滞后」不是「此刻的数据龄」，后者在数据质量页的及时性列",
  },
  dataQualityPassRate: {
    id: "dataQualityPassRate",
    label: "数据放行率（周趋势）",
    short: "每周各来源类的导入批次首次放行比例，并列该周失败的连接器运行数",
    formula: "Σok_rows ÷ (Σok_rows + Σfail_rows)，按 ISO 周 × D65 来源类；失败运行数取 integration_runs.status='failed'",
    unit: "pct",
    tier: "derived",
    caveat: "与「人工单据链准确率」同为代理口径：只说明格式/校验是否一次过，不说明单据内容对不对；分母为 0 的周留空不按 100%；不新建历史表，全部由既有运行史推导，补跑历史批次会改写过去的周读数",
  },
  suggestionRealizedRatio: {
    id: "suggestionRealizedRatio",
    label: "建议 vs 实际出库",
    short: "已成熟且独立窗口覆盖合格的建议，按 实时仓出库 ÷ 净需求 分桶计数；覆盖不足单列排除",
    formula: "每条建议视野期内实时仓 stock_ledger 出库合计（含调拨/发料，非纯销售）÷ 净需求；仅覆盖合格的成熟样本进分布，占比分母为合格行数，缺证据不按零出库",
    unit: "count",
    tier: "ledger",
    caveat: "出库 ≠ 销售需求（含调拨、发料、借出）；按原因披露排除行，覆盖检查仅涉及已登记仓与已记录日快照，不证明未接入仓或日内轨迹完整；分桶边界 50/90/110/150% 是展示约定不是考核线",
  },
});
