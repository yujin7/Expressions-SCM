...
export interface DocumentReplacementSource { id: number; docNo: string; status: string; replacement: DocumentReplacement }

/** A missing/foreign source response is unknown, never permission to create. */
export function validDocumentReplacementSource(value: unknown, id: number): value is DocumentReplacementSource {
  if (!value || typeof value !== "object") return false;
  const v = value as DocumentReplacementSource;
