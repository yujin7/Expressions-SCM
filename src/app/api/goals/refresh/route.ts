...
import { refreshableDeptKeys, refreshAutoActuals } from "@/server/modules/goals/service";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const period = typeof body.period === "string" && /^\d{4}-(\d{2}|Q[1-4])$/.test(body.period.trim()) ? body.period.trim() : undefined;
