...
/** D65 周/月核对清单：GET 列表（含当前节奏裁决）；POST generate / complete / waive（写守卫 getFreshSessionUser + service 内角色校验 + writeAudit） */
export async function GET(req: NextRequest) {
  try {
    ...
    const response = NextResponse.json({ ...list, cadence });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.action === "generate") {
      return NextResponse.json(await generateReviewPackAs(user, { periodKind: body.periodKind, periodKey: body.periodKey }, db));
    }
    const status = body.action === "complete" ? "completed" : "waived";
    return NextResponse.json(await closeReview(user, body.id, { status, note: body.note }, db));
  } catch (error) {
