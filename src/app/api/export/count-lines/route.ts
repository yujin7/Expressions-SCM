...
/**
 * 盘点明细导出（参数与盘点列表一致：period/pdId/commercialRole；无金额列，strip 为机械保障）。
 * >5000 行：自动创建异步导出任务并返回 202 {jobId}（UAT 缺口 #4）；小导出行为不变。
 */
export async function GET(req: NextRequest) {
  try {
    const user = await guardRead();
    const def = EXPORT_KINDS.countLines;
    ...
    const deferred = await syncExportGate(user, "countLines", params, total);
    if (deferred) {
    ...
    return new NextResponse(buildCsv(rows, stripMoneyColumns(columns, user.roles)), {
      headers: {
        ...
        "Content-Disposition": csvDisposition(`盘点明细_${todayShanghai()}`),
      },
    });
  } catch (e) {
