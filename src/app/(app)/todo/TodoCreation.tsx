...
export default function TodoCreation(props: Omit<TodoCreateDrawerProps, "onCancel" | "onCreated"> & { onChanged: () => void }) {
  ...
  const focus = useDialogReturnFocus(open);
  const [createdId, setCreatedId] = useState<number | null>(null);
  return <section ref={focus.scopeRef} tabIndex={-1} aria-label="待办创建">
    <Row justify="space-between" align="middle" style={{ marginBottom: 8 }}>
      <Col><Typography.Title level={4} style={{ margin: 0 }}>待办任务</Typography.Title></Col>
      <Col><Button type="primary" disabled={!ready || !!error} icon={<PlusOutlined />} onClick={event => { focus.remember(event.currentTarget); setOpen(true); }}>{request ? "恢复待办创建" : "新建待办"}</Button></Col>
    </Row>
