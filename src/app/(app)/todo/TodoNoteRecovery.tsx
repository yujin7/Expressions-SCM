...
export default function TodoNoteRecovery({ actorId }: { actorId: number }) {
  ...
  const focus = useDialogReturnFocus(!!openId);
  return <section ref={focus.scopeRef} tabIndex={-1} aria-label="待办跟进恢复">
    {error ? <Alert type="error" showIcon message={error} /> : request ? <Alert type="warning" showIcon
      message={`待办 #${request.itemId} 的原跟进结果待核对`}
      action={<Button onClick={event => { focus.remember(event.currentTarget); setOpenId(request.itemId); }}>恢复原跟进</Button>} style={{ marginBottom: 12 }} /> : null}
