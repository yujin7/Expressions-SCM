 ...
 * 而那恰好是唯一停更的应用（最新 2024-12-11）。当时的结论一度是"简道云数据太旧、
 * 需要业务授权当前视图"——**其实当前 key 一直读得到活跃数据，只是契约指错了地方**：
 * 216 张表单里 117 张含 2025 年及以后的数据，最新到 2026-08-03。
 *
...
async function main(): Promise<void> {
  ...
  console.log(`简道云可见应用 ${apps.length} 个；只列出含 ${SINCE_YEAR} 年及以后数据的表单\n`);
