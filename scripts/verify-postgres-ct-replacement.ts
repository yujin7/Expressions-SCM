/** Opt-in disposable loopback proof. Retains synthetic drafts, lineage, receipts and audits; only a registered opening fixture posts. */
...
async function main() {
  const connectionString = reviewContractConnectionString(process.env), fixture = `ct-replace-${randomUUID().slice(0, 8)}`;
  ...
  const fault = `ct_replace_fault_${randomUUID().replaceAll("-", "")}`; let faultInstalled = false;
  try {
    ...
    const [pl] = await db.insert(schema.poLines).values({ poId: po.id, skuId: sku.id, lineType: "raw", purchaseUom: "kg", uomFactor: "1", qty: "10", price: "1", receivedQty: "10" }).returning();
    // Isolated physical-stock fixture through the registered posting engine, never direct balance writes.
    await post(db, { sourceDocType: "opening", sourceDocId: 1000000000 + sku.id, action: "post",
      lines: [{ sourceLineId: 1, skuId: sku.id, warehouseId: wh.id, qtyDelta: "10" }] });
    const purchaseBefore = (await control.query("select row_to_json(p) as line from po_lines p where id=$1", [pl.id])).rows;
    const body = { poId: po.id, warehouseId: wh.id, lines: [{ poLineId: pl.id, skuId: sku.id, qty: "0.0001", batchId: null }] };
    const original = await createCt(actor, body, db);
    await voidCt(actor, original.id, { version: original.version, reason: "合成错仓，需独立重新填写" }, db);
    const originalBefore = (await control.query("select row_to_json(d) as doc from ct_docs d where id=$1", [original.id])).rows[0];
    ...
    const create = track(createCtRequest(actor, input, heldDb)); await waitBlocked(pids[1], pids[0]);
    const compete = track(createCtRequest(actor, { ...input, requestKey: randomUUID() }, otherDb));
    const repeat = track(createCtRequest(actor, input, replayDb));
    ...
    assert.deepEqual(await getCtCreateResult(actor, input.requestKey, db), created);
    assert.deepEqual(await snapshot(), { ...before, docs: before.docs + 1, receipts: 1, audits: before.audits + 1 });
    assert.deepEqual((await control.query("select row_to_json(d) as doc from ct_docs d where id=$1", [original.id])).rows[0], originalBefore);
    assert.equal((await getCt(original.id, db, actor)).replacement.successor?.id, created.document.id);
    assert.equal((await getCt(created.document.id, db, actor)).replacement.predecessor?.id, original.id);
    await assert.rejects(createCtRequest(actor, { ...input, replacementOfId: undefined }, db), conflict);
    const child = await getCt(created.document.id, db, actor);
    await voidCt(actor, child.id, { version: child.version, reason: "合成后继错单" }, db);
    await assert.rejects(createCtRequest(actor, { ...input, requestKey: randomUUID() }, db), conflict);
    ...
    await control.query(`CREATE FUNCTION ${fault}() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.user_id=${actor.id} AND NEW.entity='ct' AND NEW.action='create' THEN RAISE EXCEPTION 'synthetic replacement audit fault'; END IF; RETURN NEW; END $$`);
    ...
    await assert.rejects(createCtRequest(actor, next, db)); assert.deepEqual(await snapshot(), beforeFault);
    assert.deepEqual((await control.query("select * from doc_counters order by prefix, biz_date")).rows, countersBefore);
    assert.equal((await getCt(child.id, db, actor)).replacement.successor, null);
    await control.query(`DROP TRIGGER ${fault} ON audit_logs`); await control.query(`DROP FUNCTION ${fault}()`); faultInstalled = false;
    const grandchild = await createCtRequest(actor, next, db);
    assert.equal((await getCt(child.id, db, actor)).replacement.successor?.id, grandchild.document.id);
    const constraintError = (code: string) => (e: unknown) => (e as { code: string }).code === code;
    await assert.rejects(control.query("update ct_docs set replacement_of_id=$1 where id=$2", [original.id, grandchild.document.id]), constraintError("23505"));
    await assert.rejects(control.query("update ct_docs set replacement_of_id=id where id=$1", [grandchild.document.id]), constraintError("23514"));
    await assert.rejects(control.query("update ct_docs set replacement_of_id=-123456 where id=$1", [grandchild.document.id]), constraintError("23503"));
    const browserOriginal = await createCt(actor, body, db);
    await voidCt(actor, browserOriginal.id, { version: browserOriginal.version, reason: "合成浏览器替代验收，重新填写正确数据" }, db);
    ...
    const revoked = track(createCtRequest(actor, { ...body, replacementOfId: browserOriginal.id, requestKey: randomUUID() }, otherDb));
