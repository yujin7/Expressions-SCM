...
| PostgreSQL 发退料/结算/入库核对与自动批次并发 | `verify-postgres-settlement-basis.ts` | 手动 `SCM_ALLOW_MUTATING_PG_CONTRACT=1 DATABASE_URL=<隔离库> node --import tsx scripts/verify-postgres-settlement-basis.ts`；只允许loopback `scm_contract_*`。20项真实PG验证：既有收发/结算冻结、入库估算与失败恢复，加批次生成/工单关闭/PO数量变更/供应商暂停/账号撤权锁等待；保留合成数据，不在生产执行，不代替完整发布门 |
