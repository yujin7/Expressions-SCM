...
| PostgreSQL 采购退货错单替代 | `verify-postgres-ct-replacement.ts` | 显式`SCM_ALLOW_MUTATING_PG_CONTRACT=1`，仅已迁移0077的loopback `scm_contract_*`，关闭后台任务。四连接15项验证原单锁防分叉、同键找回、原意冲突、审计回滚、新鲜身份与关系约束。保留合成草稿/回执/审计及经注册过账器产生的一笔期初10，替代不写库存；仅移除本次故障触发器，不在正式库执行，不代替发布或真实UAT |
