...
| PostgreSQL 退料/结算依据并发 | `verify-postgres-settlement-basis.ts` | 手动 `SCM_ALLOW_MUTATING_PG_CONTRACT=1 DATABASE_URL=<隔离库> node --import tsx scripts/verify-postgres-settlement-basis.ts`；只允许loopback `scm_contract_*`。4项真实锁等待：退料与结算审批/已核对更新、累计超退、累计超发；保留合成数据，不在生产执行，不代替完整发布门 |
