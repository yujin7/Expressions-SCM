...
| PostgreSQL 新品/冻结计划回执并发 | `verify-postgres-source-replay.ts` | 手动 `SCM_ALLOW_MUTATING_PG_CONTRACT=1 DATABASE_URL=<隔离库> node --import tsx scripts/verify-postgres-source-replay.ts`；仅loopback `scm_contract_*`。7项真实锁等待覆盖同键重试、原意冲突、来源关闭、跨新品项目竞争与当前身份撤销；保留合成夹具和审计，不修改正式数据 |
