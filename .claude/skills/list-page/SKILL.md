---
name: list-page
description: Constructs list pages and client components in this supply-chain system correctly the first time — Suspense boundaries on page.tsx, the useListState/ListToolbar platform, paramPrefix isolation for multi-list pages, the client/server import boundary, search-box and empty-state conventions. Use when creating any new page under src/app/(app), adding a table or filter toolbar, adding a second list or Tab to a page that already has one, writing or editing any "use client" file, adding an import to a client component, wiring an empty state, or copying an existing -client.tsx as a template. One missing `<Suspense>` wrapper cost 24 page.tsx files a total hydration failure (commit aaf3d63 — Tabs degraded to plain text, table headers duplicated, nothing clickable) while every route still returned HTTP 200 and every unit test passed; separately, one client file value-importing @/server/* to read a label constant made 23 of 68 pages fail in a chain including /api/health. Do not use for server-side rule or number-definition changes (caliber-change, supply-chain), for schema and migration work (schema-change), or for the after-the-fact whole-app verification scan (release-sweep) — this is what you follow while writing the page, not what you run once it is written.
---
