...
describe("C122 planning version reachability and boundaries", () => {
  it("connects menu, Suspense page, client, APIs, service, rule, and persisted tables", () => {
    ...
    const peggingRoute = read("src/app/api/replenish/versions/pegging/route.ts");
    const service = read("src/server/modules/replenish/plan-versions.ts");
    const envelope = read("src/server/core/decision-envelope.ts");
    ...
    expect(client).toContain("/api/replenish/versions/pegging");
    expect(versionsRoute).toContain("capturePlanningVersion");
    expect(diffRoute).toContain("comparePlanningVersions");
    expect(peggingRoute).toContain("getPlanningPegging");
    expect(envelope).toContain("DECISION_ENVELOPE_VERSION");
    ...
    expect(postgresVerifier).toContain('"supply_demand_links:supply_demand_links_append_only"');
  });
  ...
  it("keeps capture fresh-authorized, PMC-only, idempotent, transactional, and audited", () => {
    ...
    expect(service).toContain("buildDecisionEnvelope");
    expect(service).toContain("buildSupplyDemandLinks");
  });
