 ...
 * 钉住：只比较两侧都有的完整月；锚点月（不完整）不比；单侧缺失=未覆盖不补 0；
...
import {
  computeSalesConsistency, judgeConsistency, loadSalesConsistency, refreshSalesConsistency,
...
async function seed() {
  ...
  await db.insert(schema.stagingRows).values([
    ...
    // 2026-07：A 支付 105 − 退款 5 = 100；B 200；C 3；锚点 2026-09-02 → 8 月完整、9 月不完整
    sale(1, "P-A", "2026-07-03", "60"), sale(2, "P-A", "2026-07-20", "45"),
...
describe("销量口径一致性读模型", () => {
  it("只比两侧都有的完整月；单侧缺失与锚点月不进分母；缓存随绑定失效", async () => {
    ...
    try {
      ...
      expect(r.uncovered).toEqual({ internalOnlyRows: 2, externalOnlyRows: 1 }); // D 7 月 + A 9 月；A 8 月
      ...
      expect(again.uncovered.internalOnlyRows).toBe(3);
      expect((await loadSalesConsistency(db, { channelCode: "tmall", thresholds: T })).uncovered.internalOnlyRows).toBe(3);
    } finally {
      await client.close();
    }
  });
