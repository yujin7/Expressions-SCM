...
let surface: "cockpit" | "trends" = "cockpit";
...
function render(effects = true): Node {
  for (let n = 0; n < 10; n++) {
    ...
    const tree = (surface === "cockpit" ? CockpitClient() : TrendsProbe()) as Node;
...
beforeEach(() => { resetHooks(); surface = "cockpit"; navigation.query = ""; navigation.replace.mockReset(); fetchMock.mockReset(); vi.useFakeTimers(); vi.stubGlobal("React", React); vi.stubGlobal("fetch", fetchMock); });
