...
应用发布及公网配置更新必须由同一发布宿主、同一系统运行用户执行。宿主需Python3；受控脚本按
实际Docker daemon和Compose项目取得非阻塞文件锁，忙碌返回75，应等待现有操作结束后重试。
不要删除`/tmp/exp-scm-app-operations-<uid>/`内的锁文件、靠PID/超时抢锁或手工Docker绕过。
持锁shell及继承描述符的操作进程全部退出后由内核释放；空锁文件不是“卡住”的证明。
该保护不跨主机/系统用户，跨宿主发布必须另有统一调度互斥，不能据此宣称已经具备分布式发布锁。
