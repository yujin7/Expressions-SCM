...
首次接手或需要分享资料：[团队交接与权限说明](TEAM-HANDOFF.md)。
...
- 工作区目录与本轮保留/清理边界：[知识收尾与清理审计](engineering/知识收尾与清理审计-2026-09-11.md)
- 最近库存提交修复的精确回执：[库存提交身份与事务验收](engineering/库存提交身份与事务验收-2026-09-11.md)
...
| 系统 | 在整体架构中的角色 | 当前权限边界 |
|---|---|---|
| 简道云 | 经营信号与现行人工流程观察 | 按流核验契约、范围、新鲜度、控制总量及UAT；当前证据见NOW，不在导航复制契约数量或全站可用结论 |
| 聚水潭 | 电商订单、仓配、出库与库存运营事实 | 代码和签名就绪；平台权限/IP 白名单未通过前不进入正式事实 |
| 用友 | 组织、采购、库存、成本、凭证与财务权威 | 握手、业务API权限、tenant/org范围及事实资格分层核验；当前证据见NOW |
