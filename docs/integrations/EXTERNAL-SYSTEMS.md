...
| 现行低代码 ERP 表单与历史流程 | 简道云 | 全量目录、显式表单观察、字段最小化、留证与 staging；不直接成为 SCM 正式事实 | OpenAPI 已完成只读握手；15 条契约代码就绪，其中 14 条已选运行、新增天猫费用候选默认关闭，待财务控制总量/UAT 后显式启用 |
...
账单完成行数、币种、费用项和净额控制总量/UAT 前，净毛利桥中只显示「契约已实现」，
不显示「已放行」。
