...
|---|---|---|---|
| 电商订单、实际出库销量、平台/WMS 库存观察 | 聚水潭 | 拉取、留证、映射、staging、与 SCM 自有仓出库对账 | app/token 已配置且 2026-09-03 实测仍有效；销售出库返回 `110`（开发者后台已设 IP 白名单，本机出口 IP 不在名单），店铺/仓库/库存/普通商品/采购入库返回 `190`（应用未申请这 5 条 API 权限）——两项都只能由业主在开发者后台操作，见 §解锁清单 |
| 现行低代码 ERP 表单与历史流程 | 简道云 | 全量目录、显式表单观察、字段最小化、留证与 staging；不直接成为 SCM 正式事实 | OpenAPI 已完成只读握手；33 条契约代码就绪并全部显式选择（2026-09-02 新增 4 条平台流；2026-09-03 第四阶段新增 14 条：聚水潭商品资料/组合装 BOM 镜像、天猫组合详情、拼多多商品成本标准、唯品会组合对照、财务货品档案/运营成本、唯品会商品成本、天猫 SKU 损益（90 天窗）、天猫商品流量（90 天窗）、拼多多商品/店铺日数据、店铺/品牌档案）；天猫费用已形成真实 staging 控制数，待财务 UAT 后才可提升使用等级 |
| SCM 委外单据、实时仓库存账、批次、质量、计划与审批 | 本 SCM | 业务与库存账权威 | 已运行；外部系统不得直接覆写 |
| 财务凭证、成本、结算与组织核算口径 | 用友 | 读取财务权威、提交获批业务结果、双向对账 | 客户端与 token 握手就绪（2026-09-03 复测 token 正常）；8 条只读 API 0/8，网关返回 HTTP 403 + 业务码 `310037`「API未被授权：APPKEY 未获得要调用的 API 的授权」——需在用友开发者中心把 8 条 API 逐条授权给该应用，随后填 `YY_TENANT_ID`/`YY_ORG_ID`，见 §解锁清单 |
...
## 1a. 解锁清单（2026-09-03 实测；业主在控制台操作，系统侧无需改代码）

### 聚水潭

只读探针（`writesPerformed=false`）逐接口结果：销售出库 `110`，其余 5 条 `190`。两条都是开放平台原话，
...
3. 完成后系统侧重跑只读探针（用 `JstClient.call` 逐条 `code=0` 即通过），再按 §2 顺序做真实对账/UAT，
   填 `JST_LIVE_VERIFIED_AT/REF`。token 有效期以 getInitToken 返回的 `expires_in` 为准（当前 360 天，
   2026-08-04 取得），过期前 12 小时内用 refresh 换新。

### 用友

token 握手正常；8 条只读契约全部被网关拒绝，业务码 `310037`，报文点名"APPKEY 未获得要调用的 API 的授权"。
...
- 34 条显式观察契约（2026-09-04；下文按加入批次说明；第 34 条 = 保税仓保税订单，7 天窗，不取收货人/电话/地址）：9 条核心契约覆盖产品、采购需求、采购订单、采购入库、供应商、仓库、
