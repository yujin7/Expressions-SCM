...
## 2026-09-10 只读复测与响应证据修复（候选未发布）

- 19:18（上海时间）使用正式容器的 JST/YY 配置、候选 `38314e6` 的只读探针复测；未连接 SCM 数据库、未执行平台写入，正式容器未变。
- 聚水潭 **0/6**：店铺、仓库、库存、普通商品、采购入库返回 `190`，销售出库返回 `110`。本轮令牌状态为 **未验证**，不能继承旧记录中的“token 有效”。需平台侧核对逐接口授权及出口 IP 白名单后重验。
- 用友令牌握手成功，但业务契约 **0/8**，均为 HTTP `403`；租户/组织/目标绑定尚未完整，不能声称可用或自动认领身份。需核对应用所在租户、组织、接口授权及网关访问范围；看到真实成功响应后再确认字段、分页和业务语义。
- 另以真实客户端和合成响应证伪：八个 `{}` 被旧客户端误报为8/8成功。候选修复拒绝无成功码且无对象 `data` 的响应，以及显式 `success:false`；保留已有“成功码00000”及“无码但对象data”的兼容契约。不凭空响应创造成功观察或推进 checkpoint，不改变源身份、数量、库存或财务账。
- 复测回执在仓库外交付证据 `2026-09-09-申报产能/connector-permission-20260910.json`。下文日期均保留为历史证据；当前工程验证/部署以 NOW 为准，真实授权与业务验收独立待办。

## 2026-09-06 静态复核留档

更新日期：2026-09-06（文档与源码静态复核）
本次实现锚点：本地 `agent/data-asset-decision-coverage-v2`，HEAD `0a56c4b` 及候选工作区；
部署与业务决议分别以 `docs/NOW.md`、`docs/spec/CURRENT.md` 为准，不据此声称 `main` 或运行服务已包含候选。

> 本次仅核对代码定义与留档说明，未读取密钥配置、调用外部平台或重跑探针/UAT。
> 下文 08-02、08-03、08-10、08-14、09-03 的握手、权限和业务控制数均为各自日期的历史证据，
> 不能证明 09-06 的凭据仍有效、契约已启用或连接器已通过业务验收。
...
| 事实域 | 权威系统 | SCM 的角色 | 实现与历史证据（非本轮探针） |
|---|---|---|---|
| 电商订单、实际出库销量、平台/WMS 库存观察 | 聚水潭 | 拉取、留证、映射、staging、与 SCM 自有仓出库对账 | 2026-09-03 留档：app/token 有效；销售出库返回 `110`（出口 IP 白名单），店铺/仓库/库存/普通商品/采购入库返回 `190`（API 权限）。平台侧需补齐授权，再逐接口重验，见 §解锁清单；本次未复测 |
| 现行低代码 ERP 表单与历史流程 | 简道云 | 全量目录、显式表单观察、字段最小化、留证与 staging；不直接成为 SCM 正式事实 | 已有只读握手及 staging 历史证据；契约定义以 `JIANDAOYUN_FORM_CONTRACTS` 为准（本次静态计数见 §3），实际显式选择、逐流成功与财务 UAT 分开核验，不从目录数推断全部已启用/已接通 |
| SCM 委外单据、实时仓库存账、批次、质量、计划与审批 | 本 SCM | 业务与库存账权威 | 已运行；外部系统不得直接覆写 |
| 财务凭证、成本、结算与组织核算口径 | 用友 | 按精确只读契约查询、留证、staging 与字段画像，供后续人工映射/对账；没有提交业务结果或凭证/结算回写路径 | 2026-09-03 留档：token 正常，8 条只读 API 0/8，网关拒绝授权（HTTP 403 / `310037`）。需逐接口授权并确认 tenant/org，再做映射和 UAT；本次未复测，见 §解锁清单 |
| 协同触达 | 飞书 | 接收 SCM outbox 消息；不成为业务状态权威 | 群 webhook 有真实投递与 UAT 留档；当前有效性须重新核对证据绑定和期限。过度授权的共享应用不作为生产通知身份 |
...
当时均为 HTTP `403`（早先同范围为 `310037` 未授权）。
聚水潭探针明确 `writesPerformed=false`，用友授权探针不连接
SCM 数据库，二者都不落业务数据。
当时只有飞书 webhook 的证据支持 operational；此历史结论不替代当前证据绑定与期限检查。
其余仍必须区分代码就绪、凭据、权限和 UAT。
...
## 1a. 授权解锁清单（依据 2026-09-03 历史探针；授权后仍须映射与 UAT）

### 聚水潭

2026-09-03 留档的只读探针（`writesPerformed=false`）逐接口结果：销售出库 `110`，其余 5 条 `190`。
解法只在开发者后台（openweb.jushuitan.com → 控制台 → 该自研应用）：

1. **IP 白名单**：把本机在聚水潭侧看到的出口 IP 加入名单。110 报文会直接写出当前 IP；本机境内出口与
   境外出口不同，以 110 报文里的 IP 为准，不要用境外探测站的结果。宽带 IP 会变，变了再加一条，名单允许多条。
2. **申请 API 权限**（同一应用下逐条申请）：
   - 店铺查询 `/open/shops/query`
   - 分仓查询 `/open/wms/partner/query`
   - 商品库存查询 `/open/inventory/query`
   - 普通商品查询 `/open/sku/query`
   - 销售出库查询 `/open/orders/out/simple/query`
   - 采购入库查询 `/open/purchasein/query`
3. 完成后系统侧重跑只读探针（用 `JstClient.call` 逐条 `code=0` 即通过），再按 §2 顺序做真实对账/UAT，
   填 `JST_LIVE_VERIFIED_AT/REF`。token 有效期以 getInitToken 返回的 `expires_in` 为准（2026-08-04
   取得时留档为 360 天），过期前 12 小时内用 refresh 换新。

### 用友

2026-09-03 留档：token 握手正常；8 条只读契约全部被网关拒绝，业务码 `310037`，报文点名"APPKEY 未获得要调用的 API 的授权"。本次没有再次调用。
在用友开发者中心（该应用 → API 授权/接口权限）把下列 8 条 API 加入该应用的授权范围：
...
授权后先调「分页查询当前租户组织架构」取回租户与组织 ID，填入 `.env` 的 `YY_TENANT_ID`/`YY_ORG_ID`，
再重跑 `npm run yy:probe`（期望 `p: 8, t: 8`）。`YY_SYNC_ENABLED` 仍保持 `false`，直到财务 UAT 通过。
...
### 历史验证的源事实（2026-08-02/03 留档，不代表当前授权视图）

- OpenAPI 使用 HTTPS POST JSON，`Authorization: Bearer <API_KEY>`；应用、表单、字段与数据
  分页均以官方 v5 接口读取。
- 当时 API key 可见 9 个应用、297 个表单视图。相同 `entry_id` 会在多个应用中出现，
...
- 契约目录唯一来源是 `src/server/integrations/jiandaoyun-contracts.ts` 的
  `JIANDAOYUN_FORM_CONTRACTS`；2026-09-06 静态复核为 **34 条定义**，不含独立 `catalog` 流。
  这不是运行选择或成功同步数；启用子集仍由 `JIANDAOYUN_SYNC_CONTRACTS` 显式指定。
  第 34 条 `bonded-warehouse-order-observation` 当前**没有 `window` 属性**，按非窗口观察处理；
  历史保税订单是一次性归档，不得把旧名称中的“7 天窗”当作执行契约或当前出库证明。
  以下按历史加入批次说明：9 条核心契约覆盖产品、采购需求、采购订单、采购入库、供应商、仓库、
  调拨、盘点和样品；5 条现行电商契约覆盖天猫 SKU 日销量/退款及天猫、唯品会、拼多多商品
  对照；「天猫账单费用项目汇总」只有日期×店铺×费用项粒度，不允许冒充全渠道或 SKU 直接成本；
  2026-09-02 新增 4 条（字段用 widget/list 实核、时效用服务端日期过滤探针）：天猫单品日汇总
  （子货品编码=系统编码，身份线索）、拼多多订单（**时间窗契约** `window: 3 天`，每天 3–6 千行明细，
  全量超 1,000 页上限；不取消费者资料/买家留言/商家备注）、唯品会店铺交易（品牌级）、天猫宝贝日汇总
  （商品级预估损益）。时间窗批次是滚动快照：不适用"总行数不得减少/上一批 sourceRecordId 必须仍在"
  的全量守卫，读模型跨批次按业务键去重累加（`external-velocity`、`channel-observation`）。
  2026-09-03 第四阶段再加 14 条——普查发现**聚水潭的数据早已镜像在简道云数据中台**（商品资料 6,448 项、组合装 BOM 5,244 行，
  61% 聚水潭编码与系统编码同名），加上财务维护的「系统编码↔条码↔成本标准」和运营维护的平台组合对照，直接命中当时最大的瓶颈：
  - 身份桥：`tmall-bundle-detail-observation`（天猫货品编码→子货品编码×数量，子货品编码 97% = 系统编码；对照表商家编码/关联货品 = 货品编码），
    `pdd-sku-cost-standard-observation`（商家编码→聚水潭/匹配编码），`vip-bundle-crosswalk-observation`，`finance-goods-master-observation`，
    `jst-item-master-mirror-observation`、`jst-bundle-bom-mirror-observation`；
  - 成本/毛利：`finance-operating-cost-observation`、`vip-sku-cost-observation`、`tmall-sku-cost-pnl-observation`（90 天窗）；
  - 流量先行指标：`tmall-product-traffic-observation`（90 天窗）、`pdd-product-daily-observation`（90 天窗）、`pdd-shop-daily-observation`；
  - 维度：`shop-master-observation`、`brand-master-observation`。
  实测提升（按天猫销售额）：已认领 48.6% + 待人确认精确命中 22.6% + **组合装拆解 26.8%**，真正无线索只剩 2.0%（372 个不在对照表的平台 SKU）；
  拼多多 1,281 个另一命名空间的商家编码中 246 个经成本标准翻译到系统编码；条码补齐候选 314 个（聚水潭镜像 5,160 个「国标码」里 4,127 个只是占位符 -，真条码 1,032 个；系统原只有 692/5,376 个 SKU 有条码）。
  多件组合装不做 1:1 认领，只在 `platform-sku-identity-gap`/`external-velocity` 里按数量拆到组件（观察口径）；条码补齐由人一键确认
  （`POST /api/master/sku/barcode-fill/bulk`，只写空白）。全部契约只保留供应链决策所需字段，排除联系人、手机、地址、
...
- OpenAPI 没有提供本项目可用的源快照 token，因此分页读取只声明
  `paginated-authorized-observation`，不冒充同一时点快照。重复 `data_id`、游标不前进等完整性错误
  拒绝该流；不能将一条流受阻写成整个连接器失效。每条表单流的验收提交按数据库事务锁串行化，
  目录 checkpoint 同样有流锁与较新运行隔离。`jiandaoyun-sync.ts` 按实际 `contract.window` 分支：

  - **非窗口全量观察**：非空新观察替代旧有效批次前，检查源时点、完整旧 ID 清单及记录连续性。
    同数量换 ID、用新增行掩盖未签认的旧记录缺失都会拒绝；只有通过检查的非空批次才退役旧观察。
  - **滚动窗口观察**：仍校验分页、身份与适用的源时点，不要求窗口外旧记录继续出现，
    不套用全量行数单调增长/旧 ID 全包含规则，也不退役此前窗口批次；读模型须按业务键跨批去重。
  - **空观察或失败**：空读取单独留证，不退役旧批次、不把缺失变成零；失败不提交替代结果或推进 checkpoint。
  - **逐记录删除签认（D69）已实现**：`integration_record_deletions`、
    `src/server/integrations/deletion-ack.ts` 和 `/admin/health` 卡片提供管理员专用、必填依据、同事务审计、
    可撤销的入口；不能为从未出现过的 ID 预签。全量守卫只扣除本轮实际消失且已签认的记录，
    行数下限相应减去这些记录数，不能以单次签字接受任意缩量。
  - **墓碑不绕过疑似截断**：形状检查覆盖所有消失记录，包含已签者；尾部缺失依据保存的规范 ID 顺序，
    不是原始 API 分页顺序，不能证明删除或根因。即使逐条签满，疑似截断仍拒绝；需回源核验并逐流重跑。
    D69 的具体业务签认仍待裁决，本次未签认任何记录。

  业务控制总量与第二次稳定读取仍是 UAT 必检项。真实 PostgreSQL 流锁契约验证与本次文档静态核对
  是独立证据；本次未执行该验证，更不以文档更新宣称生产重放已通过。
...
`audit-jiandaoyun-contracts` 不打开 SCM 数据库、不落源业务值，按本次选择范围输出契约聚合控制量
（指定 key 时为该子集，不指定时按当前契约目录），不是固定九条：
...
原样保留，不取绝对值。它当时已加入契约目录，但未加入当时的 14 条运行选择；在财务用平台
...
2026-08-14 留档中 token 握手成功，八条只读探针仍是 0/8（全部 HTTP 403，早先为
`310037`）；09-03 留档同样未取得八条业务读取授权。本次未请求 token 或复测权限，不能据此宣称已恢复。
代码状态应与运行状态分开：只读客户端已实现，不等于企业授权、租户组织、映射或 UAT 已就绪。
授权后先从组织查询读出租户/目标组织，再做账簿/币种/税/会计期间和控制总量 UAT。
当前唯一 API 白名单为 `src/server/integrations/yonyou-contracts.ts` 的 `YONYOU_READ_CONTRACTS`，
共 8 条只读查询；它们使用 HTTP POST，但这不代表具有业务写入能力。同步产物是原始证据、
只读观察 staging 与无值字段画像；结构漂移时显式设置 `releaseBlocked`。该标志为 false 也不证明
身份、语义或业务 UAT 已通过；`contract-consumers.ts` 尚未登记这些流的下游业务读模型。
**没有向用友提交获批业务结果、生成凭证或结算回写的现成路径**；完成读取授权也不会启用这些能力。
...
9. 先完成现有只读范围的映射、控制总量、失败重放与 UAT。未来若需要提交业务结果或财务回写，
   须另行批准精确写契约、实现 maker-checker、事务 outbox、幂等与补偿并独立验收；本清单不是回写授权，
   不存在可在只读验收后直接开启的提交开关。
...
- 简道云完成密钥轮换、重复视图裁决、每条显式启用契约的控制总量和失败恢复；全量、滚动窗口与空观察分别验收；
