...
| 现行低代码 ERP 表单与历史流程 | 简道云 | 全量目录、显式表单观察、字段最小化、留证与 staging；不直接成为 SCM 正式事实 | OpenAPI 已完成只读握手；19 条契约代码就绪并全部显式选择（2026-09-02：新增天猫单品日汇总、拼多多订单 3 天时间窗、唯品会店铺交易、天猫宝贝损益）；天猫费用已形成真实 staging 控制数，待财务 UAT 后才可提升使用等级 |
...
- 19 条显式观察契约（2026-09-02）：9 条核心契约覆盖产品、采购需求、采购订单、采购入库、供应商、仓库、
  ...
  的全量守卫，读模型跨批次按业务键去重累加（`external-velocity`、`channel-observation`）。全部契约只保留供应链决策所需字段，排除联系人、手机、地址、
